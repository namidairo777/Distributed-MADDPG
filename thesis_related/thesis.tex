...
\abstract{
Traditional reinforcement learning method such as Q-learning, Policy Gradients failed in multi-agent domain because environment becomes non-stationary during learning. And random sampling batch data from experience replay may not be effcient enough for learning. To solve these two problems, in this work, we introduce our proposed method - Distributed Multi-Agent Cooperation Algorithm besed on MADDPG algorithm\cite{maddpg} using prioritized batch data in solving Predator-Prey task. And our experiments shows we achieve 41.3\% improvement over prior MADDPG method.
}

\keywords{Multi-Agent, Reinforcement Learning, Deep Learning, Distribution, Replay Memory}
...
$ V^*(s) = \max_\pi V^\pi(s) $.
However, value function can not provide us transitions for agent to learn. Therefore, action-value function: $Q: S \times A \rightarrow R$$Q^\pi(s, a)$ was constructed:
\begin{equation}
...
\begin{equation}
y = r_t + \gamma\max Q^\pi(s_{t+1}, a_{t+1})
\end{equation}
...
However, the extensions of Policy Gradients method are mostly actor-critic method, we will introduce them in next section. 
...
\begin{figure}[h]
 \begin{center}
  ...
  \caption{
  Batch selection with max loss. (Optional!)
  }
  \label{fig:max_loss}
 \end{center}
\end{figure}
...
We could first sample $M$ size batch data, then we plan to select $N$ size batches from the sampled batch data for update. We divide $M$ size batches into $M/N$ size parts, we calculate each part's loss. We choose the part of batches with biggest loss to train. We call these good batch data as Prioritized Batch Data. \par
...
Our proposed method has 41.3\% improvement over MADDPG.
